...
## 16. Output previews and full datasets

To keep the repository lightweight while still making results easy to inspect, the project includes **CSV previews** and provides the **full Parquet outputs separately**.

### CSV previews (included in the repository)

Preview files are stored in: `outputs_preview/`

This folder also contains its own `Description.md` describing the preview datasets.

The following preview files are available:

- `entity_map_preview.csv`
- `entity_edges_evidence_preview.csv` (sample)
- `veridion_with_entity_id_preview.csv` (sample)

These previews are intentionally reduced in size:

- `entity_map_preview.csv`→ full mapping table (`record_id → entity_id`)
- `entity_edges_evidence_preview.csv`→ sample of evidence edges
- `veridion_with_entity_id_preview.csv` → sampled updated dataset

These files allow reviewers to quickly inspect the results without downloading large datasets.

---

### Full Parquet outputs 

The complete Spark outputs are provided as compressed archives:

- `entity_map_parquet.zip`
- `entity_edges_evidence_parquet.zip`
- `veridion_with_entity_id_parquet.zip`

Because Spark writes Parquet outputs as multi-file directories and the datasets can be large, they are not stored directly in the repository.

Instead, they are available in the **GitHub Releases section** of this project under:

**"Full Entity Resolution Outputs"**

These archives contain the full results generated by the pipeline.
